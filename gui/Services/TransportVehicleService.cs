...
namespace gui.Services {
    public class TransportVehicleService : BaseApiService<TransportVehicle> {
        ...
        public const string EntityPath = "TransportVehicle/";

        public TransportVehicleService() {
        ...
        public override async Task<IEnumerable<TransportVehicle>?> GetAllAsync() {
            EnsureAuthorized();
            return await ExecuteApiCallAsync<IEnumerable<TransportVehicle>>(() =>
                _httpClient.GetAsync(EntityPath));
        }

        public override async Task<TransportVehicle?> GetByIdAsync(int id) {
            EnsureAuthorized();
            return await ExecuteApiCallAsync<TransportVehicle>(() =>
                _httpClient.GetAsync($"{EntityPath}{id}"));
        }

        public override async Task AddAsync(TransportVehicle entity) {
            EnsureAuthorized();
            await ExecuteApiCallAsync(() => _httpClient.PostAsJsonAsync(EntityPath, entity));
        }

        public override async Task UpdateAsync(TransportVehicle entity) {
            EnsureAuthorized();
