...
namespace gui.Services {
    public class RoleService : BaseApiService<Role> {
        ...
        public const string EntityPath = "Role/";
        ...
        public override async Task<IEnumerable<Role>?> GetAllAsync() {
            EnsureAuthorized();
            return await ExecuteApiCallAsync<IEnumerable<Role>>(() =>
                _httpClient.GetAsync(EntityPath));
        }

        public override async Task<Role?> GetByIdAsync(int id) {
            EnsureAuthorized();
            return await ExecuteApiCallAsync<Role>(() =>
