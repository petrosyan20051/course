...
namespace gui.Services {
    public class DriverService : BaseApiService<Driver> {
        ...
        public const string EntityPath = "Driver/";
        ...
        public override async Task<IEnumerable<Driver>?> GetAllAsync() {
            EnsureAuthorized();
            return await ExecuteApiCallAsync<IEnumerable<Driver>>(() =>
                _httpClient.GetAsync(EntityPath));
        }

        public override async Task<Driver?> GetByIdAsync(int id) {
            EnsureAuthorized();
            return await ExecuteApiCallAsync<Driver>(() =>
                _httpClient.GetAsync($"{EntityPath}{id}"));
        }

        public override async Task AddAsync(Driver entity) {
            EnsureAuthorized();
